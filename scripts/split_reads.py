...
def main() -> None:
    ...
    print(f'input_bam: {args.bam_file}')
    print(f'barcode_id: {args.barcode_id}')
    print(f'region: {args.region}')

    read_count = parse_bam(args.bam_file, args.region)
    col_list, elem_count = parse_tsv(args.input_tsv)
    
    # Check region bamfile is not empty
    if read_count != 0:
        # Case 1: HOM_REF
        if elem_count == 0:
        ...
        # Case 2 - HOM_ALT
        elif elem_count != 0 and ('H1' not in col_list) or ('H2' not in col_list) and ('none' in col_list):
        ...
        print(f'{args.bam_file} is empty.')
...
# ----------------------------------------------------
def parse_bam(bam_file: str, region):
    with pysam.AlignmentFile(bam_file, 'rb') as open_fh:
        read_count = 0
        for read in open_fh.fetch(contig=region):
            read_count += 1
        return read_count
